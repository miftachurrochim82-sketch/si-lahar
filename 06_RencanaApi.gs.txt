// ============================================================
// SILAHAR - 06_RencanaApi.gs (G18b-split, 2026-09-18)
...
// Viewer dipaksa milik sendiri. mode kanban/kalender = penyajian frontend.
function ekGetRencanaList_(data, actor) {
  ...
  try {
    ...
    if (!isAdminActor_(actor)) {
      ...
      if (!myPeg) return { success: true, data: [], meta: { total: 0, page: 1, limit: 100, total_pages: 1 } };
      fPeg = myPeg;
    }
    ...
    rows.sort(function (a, b) {
      ...
      return (pr[a.prioritas] === undefined ? 3 : pr[a.prioritas]) - (pr[b.prioritas] === undefined ? 3 : pr[b.prioritas]);
    });
...
// pindah status hanya via move_status_rencana — cermin pola P2).
...
function ekMoveStatusRencana_(data, actor) {
  ...
  if (EK_ENUM.status_rencana.indexOf(tujuan) < 0) return ekFail_('BAD_REQUEST', 'Status tujuan tidak dikenal.');
  if (tujuan === 'diverifikasi') return ekFail_('FORBIDDEN', 'Status diverifikasi hanya diisi alur verifikasi.');
