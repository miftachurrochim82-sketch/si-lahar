...
- AC: buat rencana (tanggal, RHK, jenis tugas, rencana hasil, prioritas); pindah status papan: direncanakan → dikerjakan → selesai (→ diverifikasi lewat P5); view kanban (kolom status, filter tanggal/pegawai) dan view kalender bulanan; rencana kemarin belum disentuh = pengingat dashboard.
...
## P6 — Rekap Bulanan (T3)
- Story: sistem merekap capaian per pegawai per bulan; admin memicu & memantau.
- AC: generate per (pegawai, bulan) idempoten (update bila dijalankan ulang); metrik: total rencana, total realisasi, terverifikasi, revisi, capaian_pct (realisasi terverifikasi ÷ rencana), volume per RHK (json), kandidat predikat (❓rumus v1 = kuantitas saja); export Excel/PDF; daftar rekap filter periode.
