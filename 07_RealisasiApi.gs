// ============================================================
// SILAHAR - 07_RealisasiApi.gs (G18b-split, 2026-09-18)
// Domain REALISASI HARIAN: entri hasil kerja + bukti lampiran + tautan rencana.
// FR-10..FR-14.
...
function ekGetRealisasiList_(data, actor) {
  ...
  try {
    ...
    if (q) rows = rows.filter(function (r) { return matchSearch_(r, q, ['deskripsi', 'hasil', 'uraian_hasil', 'tanggal']); });
...
// Validasi + sinkron lampiran (FR-11): payload array menimpa keadaan —
// yang tak ada di payload dihapus lunak. modeValidasi=true → hanya cek
// (dipakai SEBELUM realisasi disimpan agar kegagalan lampiran tidak
// meninggalkan realisasi yatim — atomicity).
...
function ekSaveRealisasi_(data, actor) {
  ...
  try {
    ...
    if (rec.uraian_hasil !== undefined && rec.deskripsi === undefined) rec.deskripsi = rec.uraian_hasil;
    ...
    if (old) {
      ...
      if (!isAdminActor_(actor) && String(old.status_verifikasi || '').toLowerCase() === 'disetujui') {
    ...
    if (!ekMasterAktif_('JENIS_TUGAS', rec.jenis_tugas_id)) return ekFail_('BAD_REQUEST', 'jenis_tugas_id harus jenis aktif.');
    // volume + satuan: wajib untuk entri baru; update = validasi bila diisi
    var vol = rec.volume === '' || rec.volume === null || rec.volume === undefined ? null : ekNum_(rec.volume);
    if (!old) {
      if (vol === null || !(vol > 0)) return ekFail_('BAD_REQUEST', 'volume wajib numerik > 0 untuk entri baru.');
      if (!ekMasterAktif_('SATUAN', rec.satuan_id)) return ekFail_('BAD_REQUEST', 'satuan_id harus satuan aktif untuk entri baru.');
    } else {
      if (vol !== null && !(vol > 0)) return ekFail_('BAD_REQUEST', 'volume harus numerik > 0.');
    ...
    if (String(rec.rhk_id || '').trim()) {
      ...
      if (!isAdminActor_(actor) && String(rhk.pegawai_id || '').trim() !== String(rec.pegawai_id).trim()) {
...
function ekDeleteRealisasi_(data, actor) {
  ...
  if (g) return g;
  if (!isAdminActor_(actor) && String(row.status_verifikasi || '').toLowerCase() === 'disetujui') {
  ...
  ekRows_('LAMPIRAN_BUKTI').forEach(function (l) {
    if (String(l.realisasi_id || '').trim() === id) softDeleteRecordNoLock_('LAMPIRAN_BUKTI', l.id, actor, 'id');
  });
  invalidateSheetCache_('LAMPIRAN_BUKTI');
