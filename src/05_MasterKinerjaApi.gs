// ============================================================
// SILAHAR - 05_MasterKinerjaApi.gs (G18b-split, 2026-09-18; revisi G18d 2026-09-19)
// ------------------------------------------------------------
// Domain MASTER KINERJA:
//   - RHK_SKP     : kontrak kinerja tahunan per pegawai (FR-01..FR-04)
//   - JENIS_TUGAS : kamus jenis tugas (FR-02)
//   - SATUAN      : kamus satuan output (FR-03)
//
...
//     (ekEnum_, ekNum_, paginate_, ekRows_, ekFind_, ekMasterAktif_,
//     ekPegawaiDikenal_) sudah didelegasikan atau tidak butuh adopsi.
...
// Admin: bebas filter pegawai / tahun / status.
function ekGetRhkList_(data, actor) {
  ...
  try {
    ...
    if (!isAdminActor_(actor)) {
      ...
      if (!myPeg) {
        return { success: true, data: [], meta: { total: 0, page: 1, limit: 50, total_pages: 1 } };
      }
...
function ekSaveRhk_(data, actor) {
  ...
  if (!(rec.periode_tahun >= 2000 && rec.periode_tahun <= 2100)) {
    return ekFail_('BAD_REQUEST', 'periode_tahun wajib tahun valid (mis. 2026).');
  }
  ...
  if (!String(rec.nama_rhk || '').trim()) {
    return ekFail_('BAD_REQUEST', 'nama_rhk wajib diisi (kalimat HASIL).');
  }
  ...
  if (!(rec.target_tahunan > 0)) {
    return ekFail_('BAD_REQUEST', 'target_tahunan wajib numerik > 0.');
  }
...
function ekDeleteRhk_(data, actor) {
  ...
  if (ekMasterDipakai_('RENCANA_HARIAN', 'rhk_id', id) ||
      ekMasterDipakai_('LAPORAN_HARIAN', 'rhk_id', id)) {
...
function ekGetJenisTugasList_(data, actor) {
  ...
  // Non-admin selalu hanya melihat entri aktif.
  if (!data.all || !isAdminActor_(actor)) {
    rows = rows.filter(function (r) { return String(r.status || 'aktif').toLowerCase() === 'aktif'; });
  }
...
// save_jenis_tugas / save_satuan — admin; kode unik; entri terpakai:
// kode tak boleh diganti & hanya boleh dinonaktifkan (bukan dihapus).
