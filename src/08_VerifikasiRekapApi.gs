// ============================================================
// SILAHAR - 08_VerifikasiRekapApi.gs (G18b-split, 2026-09-18; revisi G18d 2026-09-19)
// ------------------------------------------------------------
...
//
// Efek verifikasi ke rencana:
//   disetujui -> rencana.status = 'diverifikasi'
//   revisi    -> rencana.status = 'dikerjakan'
//
// G18d (2026-09-19, adopsi CoreLib v2.3.0):
//   - todayIso_() → WIB (via 01_ConfigAndBridge) — tanggal_verifikasi
//     sekarang sadar zona waktu Script (sebelumnya UTC, mundur 1 hari
//     sebelum 07:00 WIB).
//   - tanggalKey10_, paginate_, ekEnum_ sudah didelegasikan (via 02/04).
...
function ekVerifikasiRealisasi_(data, actor) {
  ...
  // Normalisasi keputusan (terima 'setujui' sebagai alias 'disetujui').
  ...
  try {
    ...
    // Field verifikasi — satu-satunya penulis.
    row.status_verifikasi  = keputusan;
    row.catatan_atasan     = (data.catatan_atasan !== undefined
                              ? data.catatan_atasan
                              : (data.catatan !== undefined ? data.catatan : row.catatan_atasan)) || '';
    row.verifikator_id     = actorPegawaiId_(actor) || String(actor.id || '');
    row.tanggal_verifikasi = todayIso_(); // G18d: WIB (CoreLib.todayIsoLocal)

    var saved = writeRecordNoLock_('LAPORAN_HARIAN', row, true, actor, 'id');
    invalidateSheetCache_('LAPORAN_HARIAN');

    // Efek ke rencana terkait — cari lewat rencana_id, fallback scan realisasi_id.
    var rencana = null;
    if (String(row.rencana_id || '').trim()) {
      rencana = ekFind_('RENCANA_HARIAN', row.rencana_id);
    }
    if (!rencana) {
...
// Urut: tanggal ASC (terlama dulu — SLA 2 hari kerja).
function ekAntrianVerifikasi_(data, actor) {
  data = data || {};
  if (!isAdminActor_(actor)) {
    return ekFail_('FORBIDDEN', 'Antrian verifikasi hanya untuk admin/atasan.');
  }
...
// cache opsional {rencana, realisasi, rhk} — generate rekap banyak pegawai
// cukup baca sheet SEKALI (hindari N× pembacaan penuh).
function ekHitungMetrikBulan_(pegawaiId, periode, cache) {
  ...
  var rencana = allRencana.filter(function (r) {
    return String(r.pegawai_id || '').trim() === pegawaiId &&
           ekDalamPeriode_(r.tanggal_rencana, periode);
  });
  var realisasi = allRealisasi.filter(function (r) {
    return String(r.pegawai_id || '').trim() === pegawaiId &&
           ekDalamPeriode_(r.tanggal, periode);
  });
  ...
  var capaian = totalRencana > 0
    ? Math.round((diverifikasi / totalRencana) * 1000) / 10
    : 0;
...
function ekGenerateRekap_(data, actor) {
  ...
  try {
    ...
    pids.forEach(function (pid) {
      ...
      // Cari rekap lama (upsert by pegawai+periode)
      var lama = null;
      for (var i = 0; i < rekapAll.length; i++) {
        if (String(rekapAll[i].pegawai_id || '').trim() === pid &&
            String(rekapAll[i].periode || '') === periode) {
          lama = rekapAll[i]; break;
        }
      }
      ...
      // status_rekap: pertahankan 'final' bila sebelumnya sudah final
      rec.status_rekap = (lama && String(lama.status_rekap || '').toLowerCase() === 'final') ||
                         String(data.status_rekap || '').toLowerCase() === 'final'
                         ? 'final' : 'draf';
...
function ekGetRekapList_(data, actor) {
  ...
  try {
    ...
    rows.sort(function (a, b) {
      return String(b.periode || '') < String(a.periode || '') ? -1 : 1;
    });
